...
class UserProfile(models.Model):
    ...
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='simple')#,related_name='status')
...
class Cart(models.Model):
    ...
    def get_check_status(self):
        if self.user.status == 'gold':
            return self.get_total_price * 0.25
        elif self.status == 'silver':
            return self.get_total_price * 0.5
        elif self.status == 'bronze':
            return self.get_total_price * 0.75
        else:
